...
<div class="section">
  ...
  <div class="add-form">
    ...
    <form id="addForm" class="form-row">
      ...
      <div class="form-group cred-field" id="passGroup"><label>Password</label><input id="fPass" type="password" placeholder="password"></div>
...
function statusIcon(st) {
  const m = {active:'\\u{1F7E2}',logging_in:'\\u{1F535}',cooling:'\\u{1F534}',no_credits:'\\u{1F534}',auth_failed:'\\u{1F534}',disabled:'\\u26AB',low_credits:'\\u{1F7E1}',initializing:'\\u{1F535}'};
  return m[st] || '\\u26AA';
}
...
function renderAccounts(accounts) {
  ...
  grid.innerHTML = accounts.map(a => {
    ...
    return \`<div class="card">
      <div class="card-header">\${statusIcon(st)} <span class="card-name">\${esc(a.name)}</span></div>
...
$('#addForm').addEventListener('submit', async e => {
  ...
  if (tok) { body.token = $('#fToken').value; } else { body.email = $('#fEmail').value; body.password = $('#fPass').value; }
  try {
    ...
    else { $('#fName').value=''; $('#fEmail').value=''; $('#fPass').value=''; $('#fToken').value=''; $('#fProject').value=''; poll(); }
